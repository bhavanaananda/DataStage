# $Id: TestFileUserASharedPublic.py 1047 2009-01-15 14:48:58Z graham $
#
# Unit testing for FileUserASharedPublic module
#
...
class TestFileUserASharedPublic(unittest.TestCase):
    ...
    def testReadMeCIFS(self):
        ...
        try:
            ...
            pass
        assert (f==None), "Public user can read User A's file in shared space!"
        return
    ...
    def testCreateFileCIFS(self):
        ...
        try:
            ...
            pass
        assert (f==None), "Public user can create files in User A's shared filespace!"
        return
    ...
    def testUpdateFileCIFS(self):
        ...
        try: 
            ...
            pass
        assert (f==None), "Public user can open User A's shared files for writing!"
        return
    ...
    def testReadMeHTTP(self):
        thepage=None
        try:
            pagehandle = urllib2.urlopen(TestConfig.webdavbaseurl+'/shared/'+TestConfig.userAname+'/'+TestConfig.readmefile)
            thepage = pagehandle.read()
        except:
            pass
        assert (thepage==None), "Public user can read User A's shared file by HTTP!"
        return
    ...
    def testCreateFileHTTP(self):
        ...
        try:
            req=urllib2.Request(TestConfig.webdavbaseurl+'/shared/'+TestConfig.userAname+'/TestWebDAVCreate.tmp', data=createstring)
            req.add_header('Content-Type', 'text/plain')
            req.get_method = lambda: 'PUT'
            url=opener.open(req)
            phan=urllib2.urlopen(TestConfig.webdavbaseurl+'/shared/'+TestConfig.userAname+'/TestWebDAVCreate.tmp')
            thepage=phan.read()
        except:
            pass
        assert (thepage==None), "Public user can create a file in User A's shared filespace by HTTP!"
        return
    ...
    def testUpdateFileHTTP(self):
        ...
        try:
            req=urllib2.Request(TestConfig.webdavbaseurl+'/shared/'+TestConfig.userAname+'/TestWebDAVCreate.tmp', data=updatestring)
            req.get_method = lambda: 'PUT'
            url=opener.open(req)
            phan=urllib2.urlopen(TestConfig.webdavbaseurl+'/shared/'+TestConfig.userAname+'/TestWebDAVCreate.tmp')
            thepage=phan.read()
            ...
            pass
        assert (thepage!=updatestring), "Public user can update User A's shared file by HTTP!"
        return
...
def getTestSuite(select="unit"):
    ...
    return TestUtils.getTestSuite(TestFileUserASharedPublic, testdict, select=select)
