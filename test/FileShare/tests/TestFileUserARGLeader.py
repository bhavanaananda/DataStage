...
class TestFileUserARGLeader(unittest.TestCase):
    ...
    def testCreateFileHTTP(self):
        ...
        try:
            ...
            pass
        assert (thepage==None), "Research Group leader can create a file in User A's filespace by HTTP!"
        return
    ...
    def testUpdateFileHTTP(self):
        ...
        try:
            req=urllib2.Request(TestConfig.webdavbaseurl+'/'+TestConfig.userAname+'/TestWebDAVCreate.tmp', data=modifystring)
            req.get_method = lambda: 'PUT'
            url=opener.open(req)
            phan=urllib2.urlopen(TestConfig.webdavbaseurl+'/'+TestConfig.userAname+'/TestWebDAVCreate.tmp')
            thepage=phan.read()
            ...
            pass
        assert (thepage!=modifystring), "Research Group leader can update User A's file by HTTP!"
        return
